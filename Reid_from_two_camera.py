## 实现了两个摄像机之间的行人再识别，并记录行人来自与哪个摄像机。
...
import pandas as pd
from image_transform import Four_points_transform
##---------------------------------------------------------------------##
...
class Pedestrian_tracker:
    ...
    def getCenter(self, detection, camera_index): # detection参数为从每张图片中检测出的一个bounding box
        if camera_index == 2:
            x = (detection[2] - detection[0])/2 + 1980
        else:
    ...
    def getDistance(self, detection, index, camera_index):
        ...
        print("library is ok")
        print("camera_index is:{} centerDS is: {}".format(camera_index, self.centerDS))
        (x1, y1) = self.centerDS[index]
        print("x1 y1 read is ok")
        if camera_index == 2:
           x2 = (detection[2] - detection[0])/2 + 1920
        else:
    ...
    def setIDs(self, identifys, detections, camera_index):
        ...
        print("input of bounding box: {} data from camera:{} length of identifyDb:{}".format(len(identifys), camera_index, len(self.identifysDS)))
        ...
        for i, similarity in enumerate(similaritys):
            ...
            # update information of pedestrian
            if (similarity[detectionId] > 0.85):
                if (self.isOverlap(detections, i) == False):
                    self.identifysDS[detectionId] = identifys[i]  # 更新识别信息[1，256]
                    self.centerDS[detectionId] = self.getCenter(detections[i],camera_index)  # 更新centerDS中的位置信息
            # add new index
            # 相似度低于0.5时，更新到identifyDS中，即添加一个新成员
            elif (similarity[detectionId] < 0.3):
                if (d > 30):
                    ...
                    self.centerDS.append(self.getCenter(detections[i], camera_index))
                    ids[i] = len(self.identifysDS) - 1
                    # print("append DB size:{}".format(len(self.identifysDS)))
        print(ids)
        print("Number of pedestrian in camera:{}".format(len(self.identifysDS)))
        # 有重复编号时，去掉信赖度低的编号
...
while True:
    ...
    if (ret1 is not True) and (ret2 is not True):
        break
    # 对读取的图片透视变换
    frame1_picture_points = np.array([(200, 200), (1870, 90), (1880, 1060), (280, 1050)], dtype="float32")
    frame2_picture_points = np.array([(40, 230), (1900, 190), (1870, 1080), (20, 1080)], dtype="float32")
    # frame1 = Four_points_transform(frame1, frame1_picture_points)
    # frame2 = Four_points_transform(frame2, frame2_picture_points)
    ...
    for i, detection in enumerate(detections_cap1):
        if (ids_cap1[i] != -1):
            ...
            frame1 = cv.putText(frame1, str(ids_cap1[i]), (detection[0], detection[1]), cv.FONT_HERSHEY_PLAIN, int(30 * 0.1),
            ...
            Pedestrian_location[PedestrianID].append((center_x_cap1, center_y_cap1, detection[4],1)) #记录带有标签的boundingbox所出现的camera的编号和confidence
            # 绘制出带有ID的行人的中心位置
    ...
    #         location_start = location_list[m - 1][:2]
    #         location_end = location_list[m][:2]
    #         camera_index = location_list[m][3]  # 查看所记录的位置信息来自camera的编号
    #         distance = np.array(location_end) - np.array(location_start) # 如果出现行人位置判别偏差较大，则将异常点人为修正到正确范围
    #         point_distance = np.linalg.norm(distance)
    #         if point_distance > 10:
    #             tem_point = np.array(location_start) + np.array((5,5))
    #             tem_point = tuple(tem_point)
    #             frame1 = cv.line(frame1, location_start, tem_point, color, thickness)
    #         else:
    #             frame1 = cv.line(frame1, location_start, location_end, color, thickness)
...
# ---------------------------------------------------------#
    ...
    for i, detection in enumerate(detections_cap2):
        if (ids_cap2[i] != -1):
            ...
            Pedestrian_location[PedestrianID].append((center_x_cap2, center_y_cap2, detection[4], 2)) #记录带有标签的boundingbox所出现的camera的编号和confidence
            # 绘制出带有ID的行人的中心位置
    ...
    for i, location_list in enumerate(Pedestrian_location):
        for m in range(1, len(location_list)):
            ...
            color = box_colors[i]
            # print("color is:".format(color))
            location_start = location_list[m - 1][:2] # 提取带有index的boundingbox的x，y信息
            location_end = location_list[m][:2]
            camera_index_end = location_list[m-1][3]  # 查看所记录的位置信息来自camera的编号
            camera_index_start = location_list[m][3]  # 查看所记录的位置信息来自camera的编号
            if camera_index_start == camera_index_end:
               distance = np.array(location_end) - np.array(location_start)  # 如果出现行人位置判别偏差较大，则将异常点人为修正到正确范围
               point_distance = np.linalg.norm(distance)
               if point_distance > 10:
                  tem_point = np.array(location_start) + np.array((5, 5))
                  tem_point = tuple(tem_point)
                  if camera_index_start == 1:
                      frame1 = cv.line(frame1, location_start, tem_point, color, thickness)
                  if camera_index_start == 2:
                      frame2 = cv.line(frame2, location_start, tem_point, color, thickness)
               else:
                  if camera_index_start == 1:
                      frame1 = cv.line(frame1, location_start, location_end, color, thickness)
                  if camera_index_start == 2:
                      frame2 = cv.line(frame2, location_start, location_end, color, thickness)
...
#track_location.to_csv("Track location of Pedestrian.csv")
# Pedestrian_location.to_csv("Location of Pedestrian track2.csv")

# 将track_location的数据保存为想要的格式
location_log = pd.DataFrame(columns=["ID", "X", "Y", "Camera_index", "Confidence"])
for i, pedestrian_location in enumerate(Pedestrian_location):
