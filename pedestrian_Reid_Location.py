...
while True:
    ...
    for i, detection in enumerate(detections):
        if (ids[i] != -1):
            ...
            frame = cv.rectangle(frame, (detection[0], detection[1]), (detection[2], detection[3]), color, int(50 * 0.1))
            frame = cv.putText(frame, str(ids[i]), (detection[0], detection[1]), cv.FONT_HERSHEY_PLAIN, int(50 * 0.1),
    ...
    for i, location_list in enumerate(Pedestrian_location):
        for m in range(1, len(location_list)):
            ...
            if point_distance > 10:
                ...
                frame = cv.line(frame, location_list[m-1], tem_point, color, thickness)
            else:
                frame = cv.line(frame, location_list[m - 1], location_list[m], color, thickness)
    # 缩小画面
    h, w = frame.shape[:2]
